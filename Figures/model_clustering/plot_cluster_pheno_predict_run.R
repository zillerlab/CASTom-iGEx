...
parser$add_argument("--cohort_name_id", nargs = '*', type = "integer", help = "")
...
# type_input <- 'zscaled'
# type_cluster <- 'Cases'
# phenoRegFile <- paste0('/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/OUTPUT_GTEx/predict_CAD/Liver/200kb/CAD_GWAS_bin5e-2/Meta_Analysis_CAD/CAD_HARD_clustering/tscore_zscaled_clusterCases_phenoAssociationGLM_prediction_modelUKBB.RData')
...
# outFold <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/OUTPUT_GTEx/predict_CAD/Liver/200kb/CAD_GWAS_bin5e-2/German5/devgeno0.01_testdevgeno0/CAD_HARD_clustering/'
# ##################################################################################################################
...
for(i in 1:length(pheno_plot)){
  ...
  if(is.integer(phenoDat[, pheno_plot[i]]) & length(unique(phenoDat[, pheno_plot[i]])) <= 10){
    ...
    list_pl[[i]] <- ggplot(new_df, aes(x = gr, y = perc, fill = value, color = gr))+
      ...
      annotate("text", x = 1:P, y = rep(val_max+ val_max*0.05, P), label =tmp_reg$star_ann, size = 4)+
    ...
    if(is.numeric(phenoDat[, pheno_plot[i]]) | (is.integer(phenoDat[, pheno_plot[i]]) & length(unique(phenoDat[, pheno_plot[i]])) >= 10)){
      ...
      list_pl[[i]] <- ggplot(new_df, aes(x = gr, y = value, fill = gr))+
        ...
        xlab('')+ ylab(pheno_plot[i])+
        scale_fill_manual(values = gr_color, drop = F)+
        annotate("text", x = 1:P, y = rep(max(new_df$value) + max(new_df$value)*0.05, P), label = tmp_reg$star_ann, size = 4)+
...
ggsave(filename = sprintf('%s%s_%s_predictCluster%s_PGmethod_HKmetric_phenoSelected.png', outFold, type_data, type_input, type_cluster), width = 3.5, height = 3 + 3*(length(pheno_plot)-1), plot = tot_pl, device = 'png')
ggsave(filename = sprintf('%s%s_%s_predictCluster%s_PGmethod_HKmetric_phenoSelected.pdf', outFold, type_data, type_input, type_cluster), width = 3.5, height = 3 + 3*(length(pheno_plot)-1), plot = tot_pl, device = 'pdf')
