...
parser$add_argument("--MRRes_file", type = "character", help = "")
...
###################################################################################################################
# tissue <- 'Whole_Blood'
# MRRes_file <- paste0('OUTPUT_GTEx/predict_CAD/',tissue,'/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/enrichment_CADHARD_res/perc0.3_Mendelian_randomization_tot_path_pvalFDRrel0.05.txt')
...
MR_res <- read.delim(MRRes_file, h=T, stringsAsFactors = F, sep = '\t')
...
MR_res_red <- MR_res[match(name_exposure, MR_res$pheno),]

for(i in 1:nrow(MR_res_red)){
  
  # plot betas
  new_name <-  MR_res_red$names_field[i]
  ...
  pl <-  ggplot(df, aes(x = exp_beta, y = out_beta, label = name_plot, color = sign))+
    ...
    geom_abline(slope = MR_res_red$MREgg_est, intercept = MR_res_red$MREgg_int,
                color = 'red')+
    geom_abline(slope = MR_res_red$MREgg_est - MR_res_red$MREgg_est_se, intercept = MR_res_red$MREgg_int - MR_res_red$MREgg_int_se,
