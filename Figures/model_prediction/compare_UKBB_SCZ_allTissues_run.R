...
parser <- ArgumentParser(description="find enrichment of SCZ associated element with UKBB gene-pheno or path-pheno phenotypes")
parser$add_argument("--phenoFold", type = "character", help = "Folder with results phenotype annotation UKBB")
parser$add_argument("--inputFold_SCZ", type = "character", nargs = '*', help = "Folder with results from CAD (1 for each tissue)")
parser$add_argument("--pval_FDR_SCZ", type = "double", default = 0.05, help = "pval threshold to filter the genes and pathways (after BH correction) in CAD pheno")
...
inputFold_SCZ <- args$inputFold_SCZ
...
# tissues_name <- read.table('/psycl/g/mpsziller/lucia/PGC/eQTL_PROJECT/Meta_Analysis_SCZ/Tissues_PGC', h=F, stringsAsFactors=F)$V1[-c(2, 15)]
# inputFold_SCZ <- paste0('/psycl/g/mpsziller/lucia/PGC/eQTL_PROJECT/Meta_Analysis_SCZ/',tissues_name,'/enrichment_SCZ-UKBB_res/')
# outFold <- '/psycl/g/mpsziller/lucia/PGC/eQTL_PROJECT/Meta_Analysis_SCZ/AllTissues/enrichment_SCZ-UKBB_res/'
...
for(i in 1:length(tissues_name)){
  ...
  t <- tissues_name[i]
  print(t)
  print(sprintf('%s%s_enrichment_significant_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.RData', inputFold_SCZ[i], t, pval_FDR_SCZ, pval_FDR_UKBB))
  tmp <- get(load(sprintf('%s%s_enrichment_significant_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.RData', inputFold_SCZ[i], t, pval_FDR_SCZ, pval_FDR_UKBB)))
...
pheno_ann$pheno_type[pheno_ann$pheno_type == 'mixedpheno_Psychiatric'] <- 'Psychiatric_diseases'
...
# save total df
write.table(x = df_tscore, file = sprintf('%s/tscore_enrichment_alltissues_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.txt', outFold, pval_FDR_SCZ, pval_FDR_UKBB), 
            col.names = T, row.names = F, sep = '\t', quote = F)  
write.table(x = df_pathR, file = sprintf('%s/pathScore_Reactome_enrichment_alltissues_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.txt', outFold, pval_FDR_SCZ, pval_FDR_UKBB), 
            col.names = T, row.names = F, sep = '\t', quote = F)  
write.table(x = df_pathGO, file = sprintf('%s/pathScore_GO_enrichment_alltissues_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.txt', outFold, pval_FDR_SCZ, pval_FDR_UKBB), 
            col.names = T, row.names = F, sep = '\t', quote = F)  
...
plot_heatmap_split <- function(type_mat, mat_pval, pheno_ann, pheno_info, color_tissues, height_pl = 17, width_pl=11, cap_val = 10, split_pheno = T, type_input = 'pvalue'){
  ...
  if(split_pheno){
    ...
    mat_pval_rest <- mat_pval[, which(!id_blood & !id_blood_bio)]
    ...
    if(ncol(mat_pval_blood)>0){
      ...
      hm_pl <- pheatmap(mat=tmp_mat_capped, color=coul, show_colnames = T, show_rownames = T, 
                        ...
                        main = sprintf('%s enrichment UKBB phenotypes and SCZ\n(%s)', title_sub, type_mat),
                        breaks = mat_breaks, treeheight_row = 0, treeheight_col = 0, cellwidth = 11)
      
      
      save_pheatmap_png(hm_pl, sprintf("%s/%s_enrichment%s_Blood_count_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.png", outFold, type_mat, type_input, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl-4, width =width_pl)
      save_pheatmap_pdf(hm_pl, sprintf("%s/%s_enrichment%s_Blood_count_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.pdf", outFold, type_mat, type_input, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl-4, width =width_pl)
    }
    ...
    if(ncol(mat_pval_bloodbio)>0){
      ...
      save_pheatmap_png(hm_pl, sprintf("%s/%s_enrichment%s_Blood_biochemistry_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.png", outFold, type_mat, type_input, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl-4, width =width_pl)
      save_pheatmap_pdf(hm_pl, sprintf("%s/%s_enrichment%s_Blood_biochemistry_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.pdf", outFold, type_mat, type_input, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl-4, width =width_pl)
    }
  ...
  save_pheatmap_png(hm_pl, sprintf("%s/%s_enrichment%s_rest_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.png", outFold, type_mat, type_input, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl, width =width_pl, res = 200)
  save_pheatmap_pdf(hm_pl, sprintf("%s/%s_enrichment%s_rest_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.pdf", outFold, type_mat, type_input, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl, width =width_pl)
...
plot_heatmap_split(type_mat = 'pathScore_Reactome', mat_pval = mat_pathR_pval, pheno_ann = pheno_ann, pheno_info = pheno_info, cap_val=15, 
                   color_tissues = color_tissues, height_pl = 11, width_pl = 14)
...
plot_heatmap_split(type_mat = 'pathScore_Reactome', mat_pval = mat_pathR_OR, pheno_ann = pheno_ann, pheno_info = pheno_info, cap_val = 10, 
                   color_tissues = color_tissues, height_pl = 11, width_pl = 14, type_input = 'OR')
