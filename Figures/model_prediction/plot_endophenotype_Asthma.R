...
pheno_ann <- rbind(pheno_ann, data.frame(color = c('grey40', 'chocolate4', 'red4'), pheno_type = c('ICD9-10_OPCS4', 'Medications', 'Medical_conditions')))
pheno_ann$color[pheno_ann$pheno_type == 'Family_history'] <- 'orange3'
pheno_ann$color[pheno_ann$pheno_type == 'ICD10_Endocrine'] <- 'aquamarine4'

#######################################################################################################
setwd('/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/')
tissue <- 'Lung'
fold <- sprintf('OUTPUT_GTEx/predict_UKBB/%s/200kb/noGWAS/devgeno0.01_testdevgeno0/Asthma_pheno/Asthma_clustering/', tissue)
...
df$pheno_type <- NA
pheno_names1 <- c('Cannabis_use', 'Smoking', 'Sleep')
pheno_names2 <- c('Blood_biochemistry', 'Blood_count', 'Body_size_measures', 'Impedance_measures', 'Family_history', 'Medical_conditions', 'Alcohol', 
                  'ICD10_Respiratory_system', 'ICD10_Endocrine', 'Medications')
fold1 <- '/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/INPUT_DATA/Covariates/'
fold2 <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/'

phenoDat_endo <- list()
for(i in 1:length(pheno_names1)){
  print(pheno_names1[i])
  tmp <- fread(sprintf('%sphenotypeMatrix_%s.txt', fold1, pheno_names1[i]), h=T, stringsAsFactors=F, data.table = F)
  df$pheno_type[df$pheno_id %in% colnames(tmp)[-1]] = pheno_names1[i]
}

for(i in 1:length(pheno_names2)){
  print(pheno_names2[i])
  tmp <- fread(sprintf('%sphenotypeMatrix_%s.txt', fold2, pheno_names2[i]), h=T, stringsAsFactors=F, data.table = F)
  df$pheno_type[df$pheno_id %in% colnames(tmp)[-1]] = pheno_names2[i]
}


# make plots
id_keep <- unique(df$pheno_id[df$pvalue <= 0.00001])
...
tot_pl <- ggarrange(plotlist = list(pl_OR, pl_beta), align = 'v', ncol = 1, heights = c(1, 1.2))

ggsave(filename = sprintf('%stscore_zscaled_clusterCases_PGmethod_HKmetric_phenoAssociation_GLMpairwise_betaOR.png', fold), width = len_w+3, height = len_h*0.2+1.5, plot = tot_pl, device = 'png')
ggsave(filename = sprintf('%sstscore_zscaled_clusterCases_PGmethod_HKmetric_phenoAssociation_GLMpairwise_betaOR.pdf', fold), width = len_w+3, height = len_h*0.2+1.5, plot = tot_pl, device = 'pdf')

### plot specific phenotypes ###
library(tibble)
library(scales)

pheno_id <- c('30210', '30720', '30000', '30650')
for(i in 1:length(pheno_id)){
  ...
  if(pheno_id[i] %in% c('30210')){
    stat_tmp <- stat_tmp[grepl('gr2',stat_tmp$group1) | grepl('gr2',stat_tmp$group2) | grepl('gr4',stat_tmp$group1) | grepl('gr4',stat_tmp$group2), ]
  }
  if(pheno_id[i] %in% c('30720')){
    stat_tmp <- stat_tmp[grepl('gr5',stat_tmp$group1) | grepl('gr5',stat_tmp$group2), ]
  }
  if(pheno_id[i] %in% c('30000')){
    stat_tmp <- stat_tmp[grepl('gr1',stat_tmp$group1) | grepl('gr1',stat_tmp$group2), ]
  }
  if(pheno_id[i] %in% c('30650')){
    stat_tmp <- stat_tmp[grepl('gr3',stat_tmp$group1) | grepl('gr3',stat_tmp$group2), ]
  }
  
  stat_tmp <- cbind(stat_tmp, data.frame(.y. = rep('value', nrow(stat_tmp))))

  stat_tmp$y.position <- mean(sapply(sort(unique(res$cl$gr)), function(x) max(tmp_pheno$value[tmp_pheno$gr == paste0('gr',x)], na.rm = T))) + 0.5*(0:(nrow(stat_tmp)-1)) + 0.5
...
pheno_id <- c('41270_E03', '41270_E10', '20003_1140883066')
for(i in 1:length(pheno_id)){
  ...
  stat_tmp$group1 <- sapply(stat_tmp$comp, function(x) strsplit(x, split = '_vs_')[[1]][2])
  stat_tmp$group2 <- sapply(stat_tmp$comp, function(x) strsplit(x, split = '_vs_')[[1]][1])
  if(pheno_id[i] %in% c('41270_E03')){
    stat_tmp <- stat_tmp[grepl('gr3',stat_tmp$group1) | grepl('gr3',stat_tmp$group2), ]
  }
  if(pheno_id[i] %in% c('41270_E10', '20003_1140883066')){
    stat_tmp <- stat_tmp[grepl('gr1',stat_tmp$group1) | grepl('gr1',stat_tmp$group2), ]
  }
  stat_tmp <- cbind(stat_tmp, data.frame(.y. = rep('value', nrow(stat_tmp))))
