# find genes and pathway that are significant both for PGC and UKBB
...
parser <- ArgumentParser(description="find enrichment of PGC associated element with UKBB gene-pheno or path-pheno phenotypes")
parser$add_argument("--phenoFold", type = "character", help = "Folder with results phenotype annotation UKBB")
parser$add_argument("--inputFold_UKBB", type = "character", help = "Folder with results from UKBB all pheno association")
parser$add_argument("--inputFile_SCZ", type = "character", help = "File with results from PGC-SCZ cohort")
parser$add_argument("--pval_FDR_SCZ", type = "double", default = 0.05, help = "pval threshold to filter the genes and pathways (after BH correction) in PGC cohort")
parser$add_argument("--pval_FDR_UKBB", type = "double", default = 0.05,  help = "pval threshold to filter the genes and pathways (after BH correction) in UKBB phenotypes")
...
inputFold_UKBB <- args$inputFold_UKBB
inputFile_SCZ <- args$inputFile_SCZ
pval_FDR_SCZ <- args$pval_FDR_SCZ
pval_FDR_UKBB <- args$pval_FDR_UKBB
outFold <- args$outFold

# #########################################################################################################################
# phenoFold <- '/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/INPUT_DATA/Covariates/'
# tissue_name <- 'DLPC_CMC'
# inputFold_UKBB <- '/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/OUTPUT_CMC/predict_UKBB/200kb/devgeno0.01_testdevgeno0/'
# inputFile_SCZ <- '/psycl/g/mpsziller/lucia/PGC/eQTL_PROJECT/Meta_Analysis_SCZ/DLPC_CMC/pval_Dx_pheno_covCorr.RData'
# outFold <- '/psycl/g/mpsziller/lucia/PGC/eQTL_PROJECT/Meta_Analysis_SCZ/DLPC_CMC/enrichment_SCZ-UKBB_res/'
# pval_FDR_SCZ <- 0.05
# pval_FDR_UKBB <- 0.05 # more relaxed (sample size across phenotypes very different)
# ##########################################################################################################################

### load PGC res and filter significant ###
res_pval <- get(load(inputFile_SCZ))
tscore_PGC <- res_pval$tscore[[1]]
pathR_PGC <- res_pval$pathScore_reactome[[1]]
pathGO_PGC <- res_pval$pathScore_GO[[1]]

# filter out pathways with only 1 gene and recompute adjusted pvalue
pathR_PGC <- pathR_PGC[pathR_PGC$ngenes_tscore>1, ]
pathR_PGC$Dx_qval <- qvalue(pathR_PGC$Dx_pval)$qvalue
pathR_PGC$Dx_pval_BHcorr <- p.adjust(p = pathR_PGC$Dx_pval, method = 'BH')

pathGO_PGC <- pathGO_PGC[pathGO_PGC$ngenes_tscore>1, ]
pathGO_PGC$Dx_qval <- qvalue(pathGO_PGC$Dx_pval)$qvalue
pathGO_PGC$Dx_pval_BHcorr <- p.adjust(p = pathGO_PGC$Dx_pval, method = 'BH')

# filter based on BHcorr
tscore_PGC_red <- tscore_PGC[tscore_PGC$Dx_pval_BHcorr <= pval_FDR_SCZ,]
pathR_PGC_red <- pathR_PGC[pathR_PGC$Dx_pval_BHcorr <= pval_FDR_SCZ,]
pathGO_PGC_red <- pathGO_PGC[pathGO_PGC$Dx_pval_BHcorr <= pval_FDR_SCZ,]
...
pheno_name <- pheno_name[!pheno_name %in% c('Numeric_memory','Diffusion_brain_MRI','Medical_conditions', 'ICD10_Psychiatric', 'ICD9_Psychiatric', 'Resting_functional_brain_MRI',
                                            'dMRI_skeleton', 'T2-weighted_brain_MRI')]
pheno_info <- list()
...
for(j in 1:length(pheno_name)){
  ...
  tmp <- get(load(sprintf('%s/pval_%s_pheno_covCorr.RData', inputFold_UKBB, pheno_name[j])))
  rm(final)
  
  id_keep <- 1:nrow(tmp$pheno)
  # filter ID for each phenotype
  if(pheno_name[j] == 'Alcohol_use'){
  ...
  if(pheno_name[j] == 'Cannabis_use'){
    id_keep <- id_keep <-  which(tmp$pheno$Field %in% c('Ever taken cannabis'))
  }
  if(pheno_name[j] == 'Fluid_intelligence'){
  ...
  if(pheno_name[j] == 'Pairs_matching'){
  ...
  if(pheno_name[j] == 'Sleep'){
    id_keep <-  which(tmp$pheno$Field %in% c('Sleep duration', 'Sleepness / insomnia', 'Snoring'))
  }
  if(pheno_name[j] == 'Prospective_memory'){
    id_keep <-  which(tmp$pheno$Field %in% c('Time to answer', 'Prospective memory result'))
  }
  if(pheno_name[j] == 'Smoking'){
  ...
  if(pheno_name[j] == 'Susceptibility_weighted_brain_MRI'){
    id_keep <-  which(!tmp$pheno$Field %in% c('Discrepancy between SWI brain image and T1 brain image'))
  }
  if(pheno_name[j] == 'Symbol_digit_substitution'){
    id_keep <-  which(!tmp$pheno$Field %in% c('Symbol digit completion status'))
  }
  if(pheno_name[j] =='T1_structural_brain_MRI'){
    id_keep <- which(grepl('Volume',tmp$pheno$Field))
  }
  if(pheno_name[j] =='Task_functional_brain_MRI'){
    id_keep <- which(grepl('Median',tmp$pheno$Field) | grepl('90',tmp$pheno$Field))
  }
  if(pheno_name[j] == 'Trail_making'){
    id_keep <-  which(!tmp$pheno$Field %in% c('Trail making completion status'))
  }
  if(pheno_name[j] == 'Traumatic_events'){
    id_keep <-  which(tmp$pheno$Field %in% c('Sexually molested as a child', 'Victim of physically violent crime', 'Victim of sexual assault'))
  }
  ...
  # intersect with PGC
  tmp$tscore <- lapply(tmp$tscore[id_keep], function(x) x[x$ensembl_gene_id %in% tscore_PGC$ensembl_gene_id,])
  tmp$pathScore_reactome <- lapply(tmp$pathScore_reactome[id_keep], function(x) x[x$path %in% pathR_PGC$path,])
  tmp$pathScore_GO <- lapply(tmp$pathScore_GO[id_keep], function(x) x[x$path %in% pathGO_PGC$path,])
  ...
  test_tscore[[j]]$K <- sapply(tmp$tscore, function(x) nrow(tscore_PGC_red[tscore_PGC_red$ensembl_gene_id %in% x$ensembl_gene_id,]))
  test_pathR[[j]]$K <- sapply(tmp$pathScore_reactome, function(x) nrow(pathR_PGC_red[pathR_PGC_red$path %in% x$path,]))
  test_pathGO[[j]]$K <- sapply(tmp$pathScore_GO, function(x) nrow(pathGO_PGC_red[pathGO_PGC_red$path %in% x$path,]))
  
  test_tscore[[j]]$n <- sapply(tmp$tscore, function(x) length(which(x[,10]<=pval_FDR_UKBB)))
  test_pathR[[j]]$n <- sapply(tmp$pathScore_reactome, function(x) length(which(x[,15]<=pval_FDR_UKBB)))
  test_pathGO[[j]]$n <- sapply(tmp$pathScore_GO, function(x) length(which(x[,17]<=pval_FDR_UKBB)))
  
  test_tscore[[j]]$k <- sapply(tmp$tscore, function(x) nrow(x[x$ensembl_gene_id %in% tscore_PGC_red$ensembl_gene_id & x[, 10] <= pval_FDR_UKBB,]))
  test_pathR[[j]]$k <- sapply(tmp$pathScore_reactome, function(x) nrow(x[x$path %in% pathR_PGC_red$path & x[, 15] <= pval_FDR_UKBB,]))
  test_pathGO[[j]]$k <- sapply(tmp$pathScore_GO, function(x) nrow(x[x$path %in% pathGO_PGC_red$path & x[, 17] <= pval_FDR_UKBB,]))
  
  vect_PGC <- rep(0,nrow(tmp$tscore[[1]]))
  vect_PGC[which(tmp$tscore[[1]]$ensembl_gene_id %in% tscore_PGC_red$ensembl_gene_id)] <-1
  vect_UKBB <- lapply(tmp$tscore, function(x) rep(0,nrow(x)))
  for(k in 1:length(tmp$tscore)){
    vect_UKBB[[k]][which(tmp$tscore[[k]][,10]<=pval_FDR_UKBB)] <-1 
  }
  # compute fisher test
  id_notnull <- which(sapply(vect_UKBB, function(x) any(x == 1)))
  if(length(id_notnull)>0){
    test_tscore[[j]]$fisher_pval[id_notnull] <- sapply(vect_UKBB[id_notnull], function(x) fisher.test(x = vect_PGC, y =x, alternative = 'greater')$p.value)
    test_tscore[[j]]$fisher_OR[id_notnull] <- sapply(vect_UKBB[id_notnull], function(x) fisher.test(x = vect_PGC, y =x, alternative = 'greater')$estimate)
  }
  
  vect_PGC <- rep(0,nrow(tmp$pathScore_reactome[[1]]))
  vect_PGC[which(tmp$pathScore_reactome[[1]]$path %in% pathR_PGC_red$path)] <-1
  vect_UKBB <- lapply(tmp$pathScore_reactome, function(x) rep(0,nrow(x)))
  for(k in 1:length(tmp$pathScore_reactome)){
    vect_UKBB[[k]][which(tmp$pathScore_reactome[[k]][,15]<=pval_FDR_UKBB)] <-1 
  }
  # compute fisher test
  id_notnull <- which(sapply(vect_UKBB, function(x) any(x == 1)))
  if(length(id_notnull) > 0){
    test_pathR[[j]]$fisher_pval[id_notnull]<- sapply(vect_UKBB[id_notnull], function(x) fisher.test(x = vect_PGC, y =x, alternative = 'greater')$p.value)
    test_pathR[[j]]$fisher_OR[id_notnull]<- sapply(vect_UKBB[id_notnull], function(x) fisher.test(x = vect_PGC, y =x, alternative = 'greater')$estimate)
  }
  
  vect_PGC <- rep(0,nrow(tmp$pathScore_GO[[1]]))
  vect_PGC[which(tmp$pathScore_GO[[1]]$path %in% pathGO_PGC_red$path)] <-1
  vect_UKBB <- lapply(tmp$pathScore_GO, function(x) rep(0,nrow(x)))
  for(k in 1:length(tmp$pathScore_GO)){
    vect_UKBB[[k]][which(tmp$pathScore_GO[[k]][,17]<=pval_FDR_UKBB)] <-1 
  }
  # compute fisher test
  id_notnull <- which(sapply(vect_UKBB, function(x) any(x == 1)))
  if(length(id_notnull) > 0){
    test_pathGO[[j]]$fisher_pval[id_notnull]<- sapply(vect_UKBB[id_notnull], function(x) fisher.test(x = vect_PGC, y =x, alternative = 'greater')$p.value)
    test_pathGO[[j]]$fisher_OR[id_notnull]<- sapply(vect_UKBB[id_notnull], function(x) fisher.test(x = vect_PGC, y =x, alternative = 'greater')$estimate)
  }
  
  
  # keep only significant elements in PGC
  tmp$tscore <- lapply(tmp$tscore, function(x) x[x$ensembl_gene_id %in% tscore_PGC_red$ensembl_gene_id,])
  tmp$pathScore_reactome <- lapply(tmp$pathScore_reactome, function(x) x[x$path %in% pathR_PGC_red$path,])
  tmp$pathScore_GO <- lapply(tmp$pathScore_GO, function(x) x[x$path %in% pathGO_PGC_red$path,])
...
pheno_info$names_field[pheno_info$pheno_type == 'mixedpheno_Psychiatric'] <- pheno_info$pheno[pheno_info$pheno_type == 'mixedpheno_Psychiatric']
pheno_info$pheno_type[pheno_info$pheno_type == 'mixedpheno_Psychiatric'] <- 'Psychiatric_diseases'
...
save(res_test_enrichment, file = sprintf('%s/%s_enrichment_significant_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.RData', outFold, tissue_name, pval_FDR_SCZ, pval_FDR_UKBB))
...
id_notsign <- df_pvalcorr_tscore_all[,-c(1:2)] > pval_FDR_UKBB
df_zscore_tscore_all[,-c(1:2)][id_notsign] <- 0
id_keep <- list(row = rowSums(df_zscore_tscore_all[, -c(1:2)]) != 0, col = colSums(df_zscore_tscore_all[, -c(1:2)]) != 0)
...
##### filter out based on pvalcorr #####
id_notsign <- df_pvalcorr_pathR_all[,-c(1)] > pval_FDR_UKBB
df_zscore_pathR_all[,-c(1)][id_notsign] <- 0
id_keep <- list(row = rowSums(df_zscore_pathR_all[, -c(1)]) != 0, col = colSums(df_zscore_pathR_all[, -c(1)]) != 0)
...
##### filter out based on pvalcorr #####
id_notsign <- df_pvalcorr_pathGO_all[,-c(1)] > pval_FDR_UKBB
df_zscore_pathGO_all[,-c(1)][id_notsign] <- 0
id_keep <- list(row = rowSums(df_zscore_pathGO_all[, -c(1)]) != 0, col = colSums(df_zscore_pathGO_all[, -c(1)]) != 0)
...
save(res_zscore, file = sprintf("%s/%s_Zstatistic_intersection_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.RData", outFold, tissue_name, pval_FDR_SCZ, pval_FDR_UKBB))
...
if(tissue_name == 'DLPC_CMC'){
  write.table(x = df_color_pheno, file = sprintf('%s/color_pheno_type_UKBB.txt', phenoFold), col.names = T, row.names = F, sep = '\t', quote = F)
}

plot_heatmap_splitblood <- function(type_mat, df_zscore, PGC_res, id_to_rm, id_col, id_col_match, pheno_info, color_cat, 
                                    cap_val =9, height_pl = 20, width_pl=15){
  ...
  df_zscore_rest <- df_zscore[, which(!id_blood)]
  # split blood biochemestry
  id_blood_bio <- colnames(df_zscore) %in% pheno_info$pheno[pheno_info$pheno_type == 'Blood_biochemistry'] 
  df_zscore_blood_bio <- df_zscore[, c(id_to_rm,which(id_blood_bio))]
  
  df_zscore_rest <- df_zscore[, which(!id_blood & !id_blood_bio)]
  
  # remove genes all null
  df_zscore_blood <- df_zscore_blood[rowSums(df_zscore_blood[,-id_to_rm])!=0, ]
  df_zscore_blood_bio <- df_zscore_blood_bio[rowSums(df_zscore_blood_bio[,-id_to_rm])!=0, ]
  df_zscore_rest <- df_zscore_rest[rowSums(df_zscore_rest[,-id_to_rm])!=0,]
  
  ####################
  #### blood part ####
  ####################
  PGC_res_int <- PGC_res[match(df_zscore_blood[,1], PGC_res[,id_col_match]), ]
  # sort according to zscore and change the order in df_zscore
  PGC_res_int <- PGC_res_int[order(PGC_res_int$Dx_z, decreasing = T),]
  df_zscore_blood <- df_zscore_blood[match(PGC_res_int[,id_col_match],df_zscore_blood[,1]), ]
  ...
  if(length(dup_names)>0){
    ...
    PGC_res_int <- PGC_res_int[!PGC_res_int[, id_col] %in% dup_names,]
  }
  ...
  cl$reg[which(PGC_res_int$Dx_z<0)] = 'low'
  
  mat_row <- data.frame(reg_SCZ = cl$reg)
  rownames(mat_row) <- rownames(tmp_mat_capped)
  
  mat_colors_gr <- list(reg_SCZ = c(coul[length(coul)-1], coul[2]))
  names(mat_colors_gr$reg_SCZ) <- c('up', 'low')
  
  new_color = list(pheno = mat_colors[[1]], reg_SCZ = mat_colors_gr[[1]])
  ...
  hm_pl <- pheatmap(mat=tmp_mat_capped, color=coul, show_colnames = T, show_rownames = T, annotation_col = mat_col,  annotation_names_col = F, annotation_names_row = F, 
                    ...
                    main = sprintf('significant SCZ and significant UKBB pheno\n in %s %s', tissue_name, type_mat),
                    breaks = mat_breaks, treeheight_row = 0, treeheight_col = 0)
  
  
  save_pheatmap_png(hm_pl, sprintf("%s/%s_%s_intersection_Blood_count_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.png", outFold, tissue_name, type_mat, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl , width =width_pl-3)
  save_pheatmap_pdf(hm_pl, sprintf("%s/%s_%s_intersection_Blood_count_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.pdf", outFold, tissue_name, type_mat, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl , width =width_pl-3)
  
  #################################
  #### blood biochemestry part ####
  #################################
  
  PGC_res_int <- PGC_res[match(df_zscore_blood_bio[,1], PGC_res[,id_col_match]), ]
  # sort according to zscore and change the order in df_zscore
  PGC_res_int <- PGC_res_int[order(PGC_res_int$Dx_z, decreasing = T),]
  df_zscore_blood_bio <- df_zscore_blood_bio[match(PGC_res_int[,id_col_match],df_zscore_blood_bio[,1]), ]
  
  # remove duplicate names:
  dup_names <- names(which(table(df_zscore_blood_bio[, id_col])>1))
  if(length(dup_names)>0){
    df_zscore_blood_bio <- df_zscore_blood_bio[!df_zscore_blood_bio[, id_col] %in% dup_names,]
    PGC_res_int <- PGC_res_int[!PGC_res_int[, id_col] %in% dup_names,]
  }
  
  coul <- rev(colorRampPalette(brewer.pal(11, "RdBu"))(25))
  tmp_mat <- as.matrix(df_zscore_blood_bio[, -id_to_rm])
  rownames(tmp_mat) <- df_zscore_blood_bio[, id_col]
  val <- min(cap_val,round(max(abs(min(tmp_mat)), max(tmp_mat))))
  mat_breaks <- seq(-val, val, length.out = 25)
  pheno_tmp <- pheno_info[match(colnames(tmp_mat), pheno_info$pheno),] 
  ...
  save_pheatmap_png(hm_pl, sprintf("%s/%s_%s_intersection_Blood_biochemestry_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.png", outFold, tissue_name, type_mat, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl , width =width_pl-3)
  save_pheatmap_pdf(hm_pl, sprintf("%s/%s_%s_intersection_Blood_biochemestry_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.pdf", outFold, tissue_name, type_mat, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl , width =width_pl-3)
  
  
  ########################
  #### not blood part ####
  ########################
  PGC_res_int <- PGC_res[match(df_zscore_rest[,1], PGC_res[,id_col_match]), ]
  # sort according to zscore and change the order in df_zscore
  PGC_res_int <- PGC_res_int[order(PGC_res_int$Dx_z, decreasing = T),]
  df_zscore_rest <- df_zscore_rest[match(PGC_res_int[,id_col_match],df_zscore_rest[,1]), ]
  ...
  save_pheatmap_png(hm_pl, sprintf("%s/%s_%s_intersection_rest_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.png", outFold, tissue_name, type_mat, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl , width =width_pl)
  save_pheatmap_pdf(hm_pl, sprintf("%s/%s_%s_intersection_rest_pvalFDR-SCZ%.2f_pvalFDR-UKBB%.2f.pdf", outFold, tissue_name, type_mat, pval_FDR_SCZ, pval_FDR_UKBB), height = height_pl , width =width_pl)
  
}


# plot heatmap
plot_heatmap_splitblood(type_mat = 'tscore', id_col_match = 1, df_zscore = df_zscore_tscore_all, PGC_res = tscore_PGC_red, 
                        id_to_rm = c(1:2), id_col = 2, pheno_info = pheno_info, color_cat = color_pheno_cat)
plot_heatmap_splitblood(type_mat = 'pathScore_Reactome', id_col_match = 1, df_zscore = df_zscore_pathR_all, PGC_res = pathR_PGC_red, 
                        id_to_rm = c(1), id_col = 1, pheno_info = pheno_info, color_cat = color_pheno_cat, width_pl = 12, height_pl = 15)
plot_heatmap_splitblood(type_mat = 'pathScore_GO', id_col_match = 2, df_zscore = df_zscore_pathGO_all, PGC_res = pathGO_PGC_red, 
                        id_to_rm = c(1), id_col = 1, pheno_info = pheno_info, color_cat = color_pheno_cat, width_pl = 14.5)
