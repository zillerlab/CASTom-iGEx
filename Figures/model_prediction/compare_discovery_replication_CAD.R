# compare discovery (UKBB) and replication (CAD shunkert) dataset
# consider all possible tissues
...
for(i in 1:length(tissues_name)){
  ...
  if(pheno_name == 'CAD_SOFT'){id_pheno=2}
  if(pheno_name	== 'CAD_HARD'){id_pheno=1}
  ...
  if(nrow(df_tscore_rep[[i]])>0){
    ...
    df_tscore_disc[[i]]$CAD_rep_pval <- df_tscore_rep[[i]][,8]
    df_tscore_disc[[i]]$tissue <- tissues_name[i]
  }
  if(nrow(df_pathR_rep[[i]])>0){
    df_pathR_disc[[i]]$CAD_rep_z <- df_pathR_rep[[i]][,12]
    df_pathR_disc[[i]]$CAD_rep_pval <- df_pathR_rep[[i]][,13]
    df_pathR_disc[[i]]$tissue <- tissues_name[i]
  }
  if(nrow(df_pathGO_rep[[i]])>0){
    df_pathGO_disc[[i]]$CAD_rep_z <- df_pathGO_rep[[i]][,14]
    df_pathGO_disc[[i]]$CAD_rep_pval <- df_pathGO_rep[[i]][,15]
...
# save
save(res_tab,file = sprintf('%ssignConcordance_discovery%s_replication_signFDR%.2f.RData', outFold, pheno_name, pval_thr_FDR))
...
int_mat_allt <- c(length(which(sign(df_tscore_disc_allt[,7])*sign(df_tscore_disc_allt$CAD_rep_z) == 1)), 
                  length(which(sign(df_pathR_disc_allt[,12])*sign(df_pathR_disc_allt$CAD_rep_z) == 1)),
                  length(which(sign(df_pathGO_disc_allt[,14])*sign(df_pathGO_disc_allt$CAD_rep_z) == 1)))
...
write.table(file = sprintf('%s/tscore_discovery%s_signFDR%.2f_replicationInfo_alltissues.txt', outFold, pheno_name, pval_thr_FDR), x = df_tscore_disc_allt, quote = F, sep = '\t', col.names = T, row.names = F)
write.table(file = sprintf('%s/pathR_discovery%s_signFDR%.2f_replicationInfo_alltissues.txt', outFold, pheno_name, pval_thr_FDR), x = df_pathR_disc_allt, quote = F, sep = '\t', col.names = T, row.names = F)
write.table(file = sprintf('%s/pathGO_discovery%s_signFDR%.2f_replicationInfo_alltissues.txt', outFold, pheno_name, pval_thr_FDR), x = df_pathGO_disc_allt, quote = F, sep = '\t', col.names = T, row.names = F)
