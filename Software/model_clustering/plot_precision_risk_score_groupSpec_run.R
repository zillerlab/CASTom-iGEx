...
# 'Artery_Aorta', 'Artery_Coronary', 'Colon_Sigmoid' ,'Colon_Transverse', 'Heart_Atrial_Appendage',
# 'Heart_Left_Ventricle', 'Liver', 'Whole_Blood')
# riskScore_comp_file <- paste0('OUTPUT_GTEx/predict_CAD/',tissues,'/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_HARD_clustering/update_corrPCs/cp_riskScores_clusterCases_CAD_HARD_group_relatedPheno_measureGoodnessPred.txt')
# outFold <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/OUTPUT_GTEx/predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB/CAD_HARD_clustering/update_corrPCs/'
...
for(j in 1:length(thr_measure)){
  # print(j)
  df_repr[[j]] <- data.frame(tissue = c(tissues, 'All tissues'), n_pheno_thr = NA, 
  ...
  tmp <- df_tot
  df_repr[[j]]$n_pheno_thr[i+1] <- sum(tmp$measure >= thr_measure[j])
  df_repr[[j]]$n_same_sign[i+1] <- sum(tmp$same_sign == 1)
  
  if(sum(tmp$measure >= thr_measure[j]) > 0){
    df_repr[[j]]$precision[i+1] <- sum(tmp$measure >= thr_measure[j] & tmp$same_sign == 1)/sum(tmp$measure >= thr_measure[j])
  }else{
    df_repr[[j]]$precision[i+1] <- NA
  }
  
  if(sum(tmp$same_sign == 1) > 0){
    df_repr[[j]]$recall[i+1] <- sum(tmp$measure >= thr_measure[j] & tmp$same_sign == 1)/sum(tmp$same_sign == 1)
  }else{
    df_repr[[j]]$recall[i+1] <- NA
  }
  
}

df_repr <- do.call(rbind, df_repr)
df_repr$tissue <- factor(df_repr$tissue, levels = c(tissues, "All tissues"))
