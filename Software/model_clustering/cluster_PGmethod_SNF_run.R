...
# save results:
save(output, file = sprintf('%s%s_%s_cluster%s_PGmethod_SNF.RData', outFold, type_data, type_input, type_cluster))
...
set.seed(seed_umap)
umap_res <- uwot::umap(output$SNF_matrix[[which(kNN_par == opt_k)]], n_neighbors = n_neigh_umap, n_components = n_comp_umap, min_dist = min_dist_umap)
...
ggsave(filename = sprintf('%s%s_%s_cluster%s_PGmethod_SNF_umap.png', outFold, type_data, type_input, type_cluster), width = width_pl, height = 4, plot = tot_pl, device = 'png')
ggsave(filename = sprintf('%s%s_%s_cluster%s_PGmethod_SNF_umap.pdf', outFold, type_data, type_input, type_cluster), width = width_pl, height = 4, plot = tot_pl, device = 'pdf')
...
pl4 <- ggplot(df, aes(x = component_1, y=component_2, color = Centre))+
  ...
  theme_bw()+theme(legend.position = 'none')
tot_pl <- ggarrange(plotlist = list(pl1, pl2, pl3, pl4), ncol = 2, nrow = 2)
ggsave(filename = sprintf('%s%s_%s_cluster%s_PGmethod_SNF_umap_cov.png', outFold, type_data, type_input, type_cluster), width = 8, height = 8, plot = tot_pl, device = 'png')
ggsave(filename = sprintf('%s%s_%s_cluster%s_PGmethod_SNF_umap_cov.pdf', outFold, type_data, type_input, type_cluster), width = 8, height = 8, plot = tot_pl, device = 'pdf')
