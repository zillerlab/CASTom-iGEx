...
name_cov_tot = c(name_cov, name_cohorts)
cohort_mm <- sapply(name_cohorts, function(x) as.numeric(sampleAnn$cohort == x))
fmla <- as.formula(paste('g ~', paste0(name_cov_tot, collapse = '+')))

for(i in 1:ncol(input_data_notcorr)){
  # print(i)
  tmp <- data.frame(g = input_data_notcorr[,i], sampleAnn[, name_cov], cohort_mm)
