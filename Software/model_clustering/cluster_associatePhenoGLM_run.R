...
pl <-  ggplot(test_pheno, aes(x = new_id, y = logpval, fill = sign))+
  ...
  coord_flip()
ggsave(filename = sprintf('%s%s_%s_cluster%s_PGmethod_%smetric_phenoAssociation_GLMpairwise.png', outFold, type_data, type_input, type_cluster, type_sim), width = 4.5, height = 4, plot = pl, device = 'png')
ggsave(filename = sprintf('%s%s_%s_cluster%s_PGmethod_%smetric_phenoAssociation_GLMpairwise.pdf', outFold, type_data, type_input, type_cluster, type_sim), width = 4.5, height = 4, plot = pl, device = 'pdf')
