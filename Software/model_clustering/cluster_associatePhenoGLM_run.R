...
####################################################################################################################
# phenoDatFile <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/CAD_HARD_clustering/phenotypeMatrix_CADHARD_All.txt'
...
for(i in 1:length(gr_names)){
  ...
  # j vs all
  pheno_case_tmp <- list(phenoDat[cl == gr_names[i],], phenoDat[cl != gr_names[i],])
  covDat_tmp <- list(covDat[cl == gr_names[i],], covDat[cl != gr_names[i],]) 
