...
# riskScore_analysis_file <- c(sprintf('OUTPUT_GTEx/predict_CAD/%s/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_HARD_clustering/update_corrPCs/withMedication_riskScores_cp_tscore_corrPCs_zscaled_clusterCases_PGmethod_HKmetric_phenoAssociation_GLM.RData', tissue),
#                              sprintf('OUTPUT_GTEx/predict_CAD/%s/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_HARD_clustering/update_corrPCs/withoutMedication_riskScores_cp_tscore_corrPCs_zscaled_clusterCases_PGmethod_HKmetric_phenoAssociation_GLM.RData', tissue))
# endopheno_analysis_file <- sprintf('OUTPUT_GTEx/predict_CAD/%s/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_HARD_clustering/update_corrPCs/tscore_corrPCs_zscaled_clusterCases_PGmethod_HKmetric_phenoAssociation_GLM_combined.txt', tissue)
# outFold <- sprintf('OUTPUT_GTEx/predict_CAD/%s/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_HARD_clustering/cp_', tissue)
# pval_FDR_pheno <- 0.05
...
# R2_pheno_rs_file <- sprintf('OUTPUT_GTEx/predict_CAD/%s/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_HARD_clustering/update_corrPCs/tscore_corr2Thr0.1_relatedPhenotypes_R2_risk_score_phenotype.txt', tissue)
...
for(j in 1:length(thr_pval)){
  ...
  for(i in 1:length(comp_name)){
    ...
    pheno_c <- intersect(rs_res$pheno_id[id], endo_res_tot$pheno_id[endo_res_tot$comp %in% comp_name[i]])
    tmp <- rs_res[id, ]
...
thr <- sapply(gr_name, function(x) min(df_repr$thr_measure[df_repr$precision ==1 & df_repr$comp == x], 
                                           na.rm  = T))
if(any(!is.finite(thr))){
  gr_name_rep <- gr_name[!is.finite(thr)]
  thr <- sapply(gr_name_rep, function(x) min(df_repr$thr_measure[df_repr$precision == 0 & df_repr$comp == x], 
                                         na.rm  = T))
