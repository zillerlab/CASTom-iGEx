# ML: predict pheno status, random features seletion
...
parser <- ArgumentParser(description="ML using SNF + UMAP, select random features not used")
...
# #####################################################################################################################
# resMLFile <- 'OUTPUT_GTEx/predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB/CAD_subset/tscore_SNFUMAPp_SVM_FDRpval0.1.RData'
# tissues_name=c('Adipose_Subcutaneous' ,'Adipose_Visceral_Omentum' ,'Adrenal_Gland' ,'Artery_Aorta')
...
for(i in 1:length(tissues_name)){
  ...
  # scoreMat[[i]] <- scoreMat[[i]][,!colnames(scoreMat[[i]]) %in% unique(c(element_rm, element_rm_f))]
...
for(i in 1:length(tissues_name)){
  ...
  if(nrow(model_feat)>0){
    ...
    res_pval[[i]] <- res_pval[[i]][!res_pval[[i]][, id_info] %in% rm_feat, ]
    # exclude additional features with pvalues <=0.1
    res_pval[[i]] <- res_pval[[i]][res_pval[[i]][, id_pval]>=0.1, ]
    
    set.seed(50+i)
    res_pval[[i]] <- res_pval[[i]][sample(1:nrow(res_pval[[i]]), nrow(model_feat), replace = F), ]
    scoreMat[[i]] <- scoreMat[[i]][, match(res_pval[[i]][, id_info], colnames(scoreMat[[i]])), drop = F]
  }else{
    id_rm <- c(id_rm, i)
  }
}
...
#### apply SNF (total data) ####
SNF_tot <- SNF_matrix(tot_data, kNN = kNN_par, alpha = alphapar_SNF, tpar = tpar_SNF)
print('SNF data: completed')

#########################################################################################################################
## plot UMAP 2 components
UMAP_tot <- UMAP_sim(SNF_tot, n_comp = 2, n_neigh = n_neigh_umap, min_dist = min_dist_umap, seed_umap = seed_umap)

df <- data.frame(component_1=UMAP_tot$component_1, component_2=UMAP_tot$component_2, Dx = factor(sampleAnn$Dx))

pl1 <- ggplot(df, aes(x = component_1, y = component_2, color = Dx))+
  geom_point(size = 0.03)+
  theme_bw()+theme(legend.position = 'right')

ggsave(filename =  sprintf('%s%s_SNFUMAPp_SVM_randomFeatFDRpval%s_umap.png', outFold, type_data, as.character(pval_thr)), width = 4, height = 4, plot = pl1, device = 'png')
ggsave(filename =  sprintf('%s%s_SNFUMAPp_SVM_randomFeatFDRpval%s_umap.pdf', outFold, type_data, as.character(pval_thr)), width = 4, height = 4, plot = pl1, device = 'pdf')
...
ggsave(filename = sprintf('%s%s_SNFUMAPp_SVM_randomFeatFDRpval%s_umap_cov.png', outFold, type_data, as.character(pval_thr)), width = 12, height = 8, plot = tot_pl, device = 'png')
ggsave(filename = sprintf('%s%s_SNFUMAPp_SVM_randomFeatFDRpval%s_umap_cov.pdf', outFold, type_data, as.character(pval_thr)), width = 12, height = 8, plot = tot_pl, device = 'pdf')

print('plot UMAP data: completed')
#########################################################################################################################

#### apply UMAP ####
UMAP_tot <- UMAP_sim(SNF_tot, n_comp = n_comp_umap, n_neigh = n_neigh_umap, min_dist = min_dist_umap, seed_umap = seed_umap)
print('UMAP data: completed')

#### create SVM model ####
SVM_tot <- SVM_model(data = UMAP_tot, info_sample = sampleAnn, n_cv = 5, ncores = ncores)
print(SVM_tot$performance_nestedCV)
print('SVM data: completed')

### save ####
output <- list(sample = sampleAnn, feat = res_pval_tot, data = tot_data, par_norm = par_norm, SNF = SNF_tot, UMAP = UMAP_tot, SVM = SVM_tot)
save(output, file = sprintf('%s%s_SNFUMAPp_SVM_randomFeatFDRpval%s.RData', outFold, type_data, as.character(pval_thr)))
