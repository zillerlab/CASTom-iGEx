# ML: predict pheno status (random forest)
...
RF_tot <- RF_model(data = input_data, info_sample = sampleAnn, n_cv = 5, ncores = ncores)
print(RF_tot$performance_nestedCV)
print(RF_tot$performance_CV)
print('RF data: completed')

### save ####
output <- list(sample = sampleAnn, feat = res_pval_tot, data = tot_data, par_norm = par_norm, RF = RF_tot)
save(output, file = sprintf('%s%s_%s_RF_FDRpval%s.RData', outFold, type_data, type_input, as.character(pval_thr)))
