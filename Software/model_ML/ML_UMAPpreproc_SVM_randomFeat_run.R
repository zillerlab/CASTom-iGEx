...
# inputFile <- paste0('OUTPUT_GTEx/predict_CAD/',tissues_name,'/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_subset/Pathway_Reactome_scores.RData')
...
tot_data_j <- do.call(cbind, tot_data)
colnames(tot_data_j) <- res_pval_tot$new_id

#########################################################################################################################
## plot UMAP 2 components
UMAP_tot <- UMAP_model(tot_data_j, n_comp = 2, n_neigh = n_neigh_umap, min_dist = min_dist_umap, seed_umap = seed_umap)

df <- data.frame(component_1=UMAP_tot$data$component_1, component_2=UMAP_tot$data$component_2, Dx = factor(sampleAnn$Dx))

pl1 <- ggplot(df, aes(x = component_1, y = component_2, color = Dx))+
  geom_point(size = 0.03)+
  theme_bw()+theme(legend.position = 'right')

ggsave(filename =  sprintf('%s%s_UMAPp_SVM_randomFeatFDRpval%s_umap.png', outFold, type_data, as.character(pval_thr)), width = 4, height = 4, plot = pl1, device = 'png')
ggsave(filename =  sprintf('%s%s_UMAPp_SVM_randomFeatFDRpval%s_umap.pdf', outFold, type_data, as.character(pval_thr)), width = 4, height = 4, plot = pl1, device = 'pdf')
...
ggsave(filename = sprintf('%s%s_UMAPp_SVM_randomFeatFDRpval%s_umap_cov.png', outFold, type_data, as.character(pval_thr)), width = 12, height = 8, plot = tot_pl, device = 'png')
ggsave(filename = sprintf('%s%s_UMAPp_SVM_randomFeatFDRpval%s_umap_cov.pdf', outFold, type_data, as.character(pval_thr)), width = 12, height = 8, plot = tot_pl, device = 'pdf')
...
#### apply UMAP ####
UMAP_tot <- UMAP_model(tot_data_j, n_comp = n_comp_umap, n_neigh = n_neigh_umap, min_dist = min_dist_umap, seed_umap = seed_umap)
print('UMAP data: completed')

#### create SVM model ####
SVM_tot <- SVM_model(data = UMAP_tot$data, info_sample = sampleAnn, n_cv = 5, ncores = ncores)
print(SVM_tot$performance_nestedCV)
print('SVM data: completed')

### save ####
output <- list(sample = sampleAnn, feat = res_pval_tot, data = tot_data, par_norm = par_norm, UMAP = UMAP_tot, SVM = SVM_tot)
save(output, file = sprintf('%s%s_UMAPp_SVM_randomFeatFDRpval%s.RData', outFold, type_data, as.character(pval_thr)))
