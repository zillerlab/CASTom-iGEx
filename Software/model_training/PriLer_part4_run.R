...
resNoPrior_Her_nCV <- get(load(sprintf('%sresNoPrior_NestedCV_HeritableGenes_%s.RData', part1Res_fold, curChrom)))
...
if(which.min(colMeans(err_test))<ncol(err_test)){
  ...
  resPrior_Her <- get(load(sprintf('%sresPrior_EOpt_HeritableGenes_allchr.RData', part3Res_fold)))
}else{
  id_opt <- which(abs(diff(colMeans(err_test))) < 0.5)[1] + 1
  E_hat <- as.numeric(strsplit(names(id_opt), 'E_h.')[[1]][2])
  E_par <- E_hat
  resPrior_Her_nCV <- get(load(sprintf('%sresPrior_EFixed%.2f_NestedCV_HeritableGenes_allchr.RData', part2Res_fold, E_par)))
  resPrior_Her <- get(load(sprintf('%sresPrior_EFixed%.2f_HeritableGenes_allchr.RData', part3Res_fold, E_par)))
}
...
# seed/nfold CV
resNoPrior_Her <- get(load(sprintf('%sresNoPrior_HeritableGenes_allchr.RData', part3Res_fold)))
...
for(k in 1:nfolds_out){
  ...
  colnames(res_nop[[k]]) <- c('gene', 'idBeta', 'Beta', 'lambda', 'alpha', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')
  ...
  colnames(test_res_nop[[k]]) <- c('dev', 'dev_geno','dev_cov', 'dev_geno_cov', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')
  ...
  colnames(res_p[[k]]) <- c('gene', 'idBeta', 'Beta', 'lambda', 'alpha', 'SquErr', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')
  ...
  colnames(test_res_p[[k]]) <- c('dev', 'dev_geno','dev_cov', 'dev_geno_cov', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')
...
cor_comb_noadj_test_nop <- data.frame(cor = rep(0,N), cor_pval = rep(0,N))
for(g in 1:N){
  cor_comb_noadj_test_nop$cor[g] <- cor.test(ExpTest_comb[g,],predTest_geno_comb_nop[g,])$estimate
  cor_comb_noadj_test_nop$cor_pval[g] <- cor.test(ExpTest_comb[g,],predTest_geno_comb_nop[g,])$p.value
}
...
cor_comb_noadj_test_p <- data.frame(cor = rep(0,N), cor_pval = rep(0,N))
for(g in 1:N){
  cor_comb_noadj_test_p$cor[g] <- cor.test(ExpTest_comb[g,],predTest_geno_comb_p[g,])$estimate
  cor_comb_noadj_test_p$cor_pval[g] <- cor.test(ExpTest_comb[g,],predTest_geno_comb_p[g,])$p.value
}

train_res_nop <- lapply(res_nop, function(x) x[!duplicated(x$gene), colnames(x) %in% c('lambda','alpha', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')])
train_res_p <- lapply(res_p, function(x) x[!duplicated(x$gene), colnames(x) %in% c('dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')])

res_tot_nop <- list(geneAnn = gene_ann, train = train_res_nop, test = test_res_nop, 
                    cor_comb_test = cor_comb_test_nop, cor_comb_noadj_test = cor_comb_noadj_test_nop,
...
res_tot_p <- list(geneAnn = gene_ann, train = train_res_p, test = test_res_p, 
                  cor_comb_test = cor_comb_test_p, cor_comb_noadj_test = cor_comb_noadj_test_p,
                  beta_snps = betaSnpsMatrix_p, beta_cov = betaCovMatrix_p, 
                  seed = data.frame(type = c('inner', 'outer'), value = c(seed_in, seed_out), nfold = c(nfolds_in, nfolds_out)))

# store results
save(res_tot_nop, file = sprintf('%sresNoPrior_NestedCV_NotHeritableGenes_%s.RData', outFold, curChrom))
save(res_tot_p, file = sprintf('%sresPrior_NestedCV_NotHeritableGenes_%s.RData', outFold,  curChrom))
...
colnames(res_nop_alls) <- c('gene', 'idBeta', 'Beta', 'lambda', 'alpha', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval','cor_noadj', 'cor_noadj_pval')
...
tot_nop_alls <- res_nop_alls[!duplicated(res_nop_alls$gene), colnames(res_nop_alls) %in% c('lambda', 'alpha', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval','cor_noadj', 'cor_noadj_pval')]
...
save(res_tot_nop, file = sprintf('%sresNoPrior_NotHeritableGenes_%s.RData', outFold,  curChrom))
...
colnames(res_p_alls) <- c('gene', 'idBeta', 'Beta', 'lambda', 'alpha', 'SquErr', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')
...
tot_p_alls <- res_p_alls[!duplicated(res_p_alls$gene), colnames(res_p_alls) %in% c('lambda', 'alpha', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval','cor_noadj', 'cor_noadj_pval')]

res_tot_p <- list(geneAnn = gene_ann, tot = tot_p_alls, beta_snps = betaSnpsMatrix_p_tot, beta_cov = betaCovMatrix_p_tot, 
                  seed = data.frame(type ='tot', value = seed, nfold = nfolds)) 

save(res_tot_p, file = sprintf('%sresPrior_NotHeritableGenes_%s.RData', outFold, curChrom))
