...
parser$add_argument("--seed", type="integer", default = 4321, help = "seed to create folders [default %(default)s]")
parser$add_argument("--nfolds", type="integer", default = 5, help = "n. of folds [default %(default)s]")
...
cv_test_file <-sprintf('%scv_test_Epar_allchr.txt', part2Res_fold)
...
for(i in 1:length(all_Chroms)){
  ...
  # order based on sample ann
  id <- match(sampleAnn$genoSample_ID, colnames(tmp))
  tmp <- tmp[, id]
...
for(chr in 1:length(all_Chroms)){
  ...
  colnames(res_noprior[[chr]] ) <- c('gene', 'idBeta', 'Beta', 'lambda', 'alpha', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')
...
allsamples_res <- lapply(res_noprior, function(x) x[!duplicated(x$gene), colnames(x) %in% c('lambda', 'alpha', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')])
...
save(res_tot_nop, file = sprintf('%sresNoPrior_HeritableGenes_allchr.RData', outFold))
...
corFin_noadj <- matrix(0,maxIter,N)
corFin_noadj_pval <- matrix(0,maxIter,N)
...
while ((cIter <= maxIter) & (!converged)){
  ...
  for(j in 1:length(all_Chroms)){
    ...
    colnames( res[[j]] ) <- c('gene', 'idBeta', 'Beta', 'lambda', 'alpha',  'SquErr', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval', 'cor_noadj', 'cor_noadj_pval')
  ...
  corFin_noadj[cIter,id_notzero] <- unlist(sapply(res, function(x) x[!duplicated(x[,1]),  colnames(x) == 'cor_noadj']))
  corFin_noadj_pval[cIter,id_notzero] <- unlist(sapply(res, function(x) x[!duplicated(x[,1]),  colnames(x) == 'cor_noadj_pval']))
...
corFin_noadj <- corFin_noadj[1:nIter, ]
corFin_noadj_pval <- corFin_noadj_pval[1:nIter,]
...
results_iteration <- list(Eopt = E_hat, pWeight = weightMat, errComp = fStats, nCount = nCountList, 
                          dev = devFin, dev_geno = devFin_geno, dev_cov = devFin_cov, dev_genocov = devFin_genocov, 
                          cor = corFin, cor_pval = corFin_pval, 
                          cor_noadj = corFin_noadj, cor_noadj_pval = corFin_noadj_pval, 
                          MSE = SquErr, beta = BetaIt, obj = tot_obj)

if(E_par<0){
  save(x = results_iteration, file = sprintf('%sresPrior_EOpt_Iteration_HeritableGenes_allchr.RData', outFold))  
}else{
  save(x = results_iteration, file = sprintf('%sresPrior_EFixed%.2f_Iteration_HeritableGenes_allchr.RData', outFold, E_par))
}
...
if(E_par<0){
  save(x = res_tot_p, file = sprintf('%sresPrior_EOpt_HeritableGenes_allchr.RData', outFold))  
}else{
  save(x = res_tot_p, file = sprintf('%sresPrior_EFixed%.2f_HeritableGenes_allchr.RData', outFold, E_par))
}
