...
################################################################
# cis_thres = 200000
# part1Res_fold = '/ziller/lucia/eQTL_PROJECT_CMC/OUTPUT_CMC_SCRIPTS_v2/train_Control50/200kb/'
# part2Res_fold = part3Res_fold = part4Res_fold =  '/ziller/lucia/eQTL_PROJECT_CMC/OUTPUT_CMC_SCRIPTS_v2/train_Control50/200kb/'
# outFold <- '/ziller/lucia/eQTL_PROJECT_CMC/OUTPUT_CMC_SCRIPTS_v2/train_Control50/200kb/'
# InfoFold <- '/ziller/lucia/eQTL_PROJECT_CMC/OUTPUT_CMC_SCRIPTS_v2/'
# functR <- '/ziller/lucia/eQTL_PROJECT_CMC/RSCRIPTS/SCRIPTS_v2/ElNet_withPrior_functions_run.R'
# priorDat_file <- '/ziller/lucia/eQTL_PROJECT_CMC/OUTPUT_CMC_SCRIPTS_v2/priorMatrix_'
# priorInf <- c(18,2:15)
# covDat_file <- '/ziller/lucia/eQTL_PROJECT_CMC/OUTPUT_CMC_SCRIPTS_v2/train_Control50/covariateMatrix_Control50.txt'
################################################################
...
if(id_con == id_opt){
  res_p_her_tot_allchr <- get(load(sprintf('%s/resPrior_EOpt_HeritableGenes_allchr.RData', part3Res_fold)))
}else{
  res_p_her_tot_allchr <- get(load(sprintf('%s/resPrior_EFixed%.2f_HeritableGenes_allchr.RData', part3Res_fold, Epar[id_con])))
}

res_nop_her_tot_allchr <- get(load(sprintf('%s/resNoPrior_HeritableGenes_allchr.RData',part3Res_fold)))
res_p_her_allchr <- get(load(sprintf('%s/resPrior_EOpt_NestedCV_HeritableGenes_allchr.RData',part2Res_fold)))
...
for(i in all_Chroms){
  ...
  res_nop_her <- get(load(sprintf('%s/resNoPrior_NestedCV_HeritableGenes_chr%i.RData',part1Res_fold, i)))
  res_nop_nother <- get(load(sprintf('%s/resNoPrior_NestedCV_NotHeritableGenes_chr%i.RData',part4Res_fold, i)))
  res_nop_nother_tot <- get(load(sprintf('%s/resNoPrior_NotHeritableGenes_chr%i.RData',part4Res_fold, i)))
  ...
  res_p_nother <- get(load(sprintf('%s/resPrior_NestedCV_NotHeritableGenes_chr%i.RData',part4Res_fold, i)))
  res_p_nother_tot <- get(load(sprintf('%s/resPrior_NotHeritableGenes_chr%i.RData', part4Res_fold, i)))
  ...
  res_p_her <- list(geneAnn = res_p_her_allchr$geneAnn[id_chr,], train = lapply(res_p_her_allchr$train_opt, function(x) x[id_chr,]), 
                    test = lapply(res_p_her_allchr$test_opt, function(x) x[id_chr,]), cor_comb_test = res_p_her_allchr$cor_comb_test_opt[id_chr,])
...
# save results
write.table(file = sprintf('%s/resNoPrior_regEval_allchr.txt', outFold), x = res_nop, col.names = T, row.names = F, sep = '\t', quote = F)
write.table(file = sprintf('%s/resPrior_regEval_allchr.txt', outFold), x = res_p, col.names = T, row.names = F, sep = '\t', quote = F)

write.table(file = sprintf('%s/resNoPrior_regCoeffCov_allchr.txt', outFold), x = beta_cov_nop, col.names = T, row.names = F, sep = '\t', quote = F)
write.table(file = sprintf('%s/resPrior_regCoeffCov_allchr.txt', outFold), x = beta_cov_p, col.names = T, row.names = F, sep = '\t', quote = F)

save(beta_snps_nop, file = sprintf('%s/resNoPrior_regCoeffSnps_allchr.RData', outFold))
save(beta_snps_p, file = sprintf('%s/resPrior_regCoeffSnps_allchr.RData', outFold))
...
df_train <- data.frame(mean_err = colMeans(err_train), E_par = Epar, sd_err = apply(err_train,2,sd))
df_test <- data.frame(mean_err = colMeans(err_test), E_par = Epar, sd_err = apply(err_test,2,sd), opt = rep('not', length(Epar)), stringsAsFactors = F)
df_test$opt[id_opt] <- 'opt'

if(id_opt == length(Epar)){
  df_test$opt[id_con] <- 'conv'   
}
...
plot_errbars<- ggplot(data = df_tot, aes(x = E_par, y = mean_err)) +
  ...
  scale_x_continuous("E par", labels = as.character(Epar), breaks = Epar)
...
res_objcv_train <- get(load(sprintf('%s/obj_Epar_cvtrain_allchr.RData', outFold)))
...
res_objcv_test <- get(load(sprintf('%s/obj_Epar_cvtest_allchr.RData', outFold)))
...
file_name <- sprintf('%s/plot_CVdev_trainVStest_allgenes.png', outFold)
ggsave(file_name, ggMarginal(pl_devcv, groupColour = TRUE, groupFill = TRUE), width = 5, height = 5, units = "in")
...
file_name <- sprintf('%s/plot_CVdevgeno_trainVStest_allgenes.png', outFold)
ggsave(file_name, ggMarginal(pl_devgenocv, groupColour = TRUE, groupFill = TRUE), width = 5, height = 5, units = "in")
...
file_name <- sprintf('%s/plot_devtrain_compareWithPrior_increase.png', outFold)
ggsave(file_name, pl_genonew_train, width = 4, height = 4, units = "in", dpi = 500)
...
file_name <- sprintf('%s/plot_devtest_compareWithPrior_increase.png', outFold)
ggsave(file_name, pl_genonew_test, width = 4, height = 4, units = "in", dpi = 500)
...
file_name <- sprintf('%s/boxplot_devainceDecomposition_elVSelP_allgenes.png', outFold)

ggsave(plot = pl_box, file = file_name, width = 5, height = 6)
...
file_name <- sprintf('%s/plot_compare_dev_elVSelP_allgenes.png', outFold)

ggsave(file_name, pl_dev, width = 4.5, height = 4.5, units = "in")
...
file_name <- sprintf('%s/plot_compare_devgeno_elVSelP_allgenes.png', outFold)
...
for(i in 1:length(all_Chroms)){
  ...
  snpPos[[i]] <- read.table(sprintf('%s/hg19_SNPs_%s_matched.txt', InfoFold, chr), header = T, sep = '\t')
...
file_name <- sprintf('%s/plot_numberSNPs_elVSelP_gene.png', outFold)
ggsave(file_name, pl_hist, width = 5.1, height = 5.1, units = "in", dpi = 500)

write.table(file = sprintf('%s/nVariants_prior_compare.txt', outFold),x = df_len, col.names = T, row.names = F, sep ='\t', quote = F)
...
file_name <- sprintf('%s/plot_priorCoeff_dist.png', outFold)
ggsave(file_name, totdensity_p, width = 7, height = 5, units = "in")
...
file_name <- sprintf('%s/plot_priorFeatContr_dist.png', outFold)
ggsave(file_name, totdensity_feat, width = 7, height = 5, units = "in")
...
file_name <- sprintf('%s/plot_weights_priorFeat.png', outFold)
ggsave(file_name,  pl_w, width = 6, height = 2+0.1*length(pNames), units = "in")
...
if(id_con == id_opt){
  res_p_her_tot_allchr_it <- get(load(sprintf('%s/resPrior_EOpt_Iteration_HeritableGenes_allchr.RData', part3Res_fold)))
}else{
  res_p_her_tot_allchr_it <- get(load(sprintf('%s/resPrior_EFixed%.2f_Iteration_HeritableGenes_allchr.RData', part3Res_fold, Epar[id_con])))
}
...
file_name <- sprintf('%splot_iteration_weigths_priorFeat.png', outFold)
ggsave(filename =file_name, plot = pl_it, width = 6, height = 4, dpi=500)
...
write.table(file = sprintf('%s/fractionPrior_perFeature.txt', outFold),x = df_prior, col.names = T, row.names = F, sep ='\t', quote = F)
...
file_name <- sprintf('%s/plot_priorFraction_elVSelP.png', outFold)
ggsave(file_name,   pl_prior, width = 5.1, height = 5.1, units = "in", dpi = 500)

file_name <- sprintf('%s/plot_priorFraction_elVSelP_legend.png', outFold)
ggsave(file_name,   pl_leg, width = 5.1, height = 5.1, units = "in", dpi = 500)
...
file_name <- sprintf('%s/plot_distAbsBeta_regSNPsPrior_elANDelP.png', outFold)
ggsave(file_name,   pl_abs, width = 8, height = 5.1, units = "in", dpi = 500)
...
file_name <- sprintf('%s/plot_AbsBeta_regSNPsPrior_elVSelP.png', outFold)
ggsave(file_name, pl_med, width = 5.1, height = 5.1, units = "in", dpi = 500)
