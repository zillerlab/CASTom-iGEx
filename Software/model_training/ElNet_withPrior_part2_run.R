...
parser$add_argument("--E_set", type="double", nargs = '*', help = "range E parameter to search, if length = 1 only 1 value tested")
...
for(i in 1:length(all_Chroms)){
  ...
  lambda_file <- sprintf('%s/optim_lambda_%s.txt', part1Res_fold, chr)
  print(lambda_file)
  alpha_file <- sprintf('%s/optim_alpha_%s.txt', part1Res_fold, chr)
  print(alpha_file)
  ...
  res_part1 <- get(load(sprintf('%s/resNoPrior_NestedCV_HeritableGenes_%s.RData', part1Res_fold, chr)))
...
res_E <- foreach(l=1:length(E_set), .combine='comb', .multicombine=TRUE, 
                 .init=list(list(), list(), list(), list(),list(),list(), list(), list(), list(), list(), list(),list(), list(),
                            list(), list(), list(), list(), list(), list(),list(), list(), list(), list(), list(), list()))%dopar%{
  ...
  for(k in 1:nfolds_out){
    ...
    while ((cIter <= maxIter) & (!converged)){
      ...
      for(j in 1:length(all_Chroms)){
        ...
        colnames(res_k[[j]]) <- c('gene', 'idBeta', 'Beta', 'lambda', 'alpha',  'SquErr', 'dev', 'dev_geno', 'dev_cov', 'dev_geno_cov', 'dev_lmgeno', 'cor', 'cor_pval')
  ...
  list(weightMat, fStats_train, fStats_test, nCountList, SquErr, tot_obj, tot_obj_test, nIter,  
       devTrain, devTrain_geno, devTrain_cov, devTrain_genocov, devTrain_lmgeno, corTrain, corTrain_pval, 
       devTest, devTest_geno, devTest_cov, devTest_genocov, corTest, corTest_pval, 
       predTest_geno_fin, predTest_cov_fin, betaSnpsMatrix, betaCovMatrix)
  
}  

# save total result
save(res_E, file = sprintf('%s/resE_allchr.RData', outFold))
...
betaSnpsMatrix <- res_E[[24]]
betaCovMatrix <- res_E[[25]]
...
for(l in 1:length(E_set)){
  ...
  dev_test[[l]] <- do.call(cbind, lapply(1:nfolds_out, function(k) res_E[[16]][[l]][[k]][nIter[[l]][k],])) # dim Nxnfolds
  dev_geno_test[[l]] <- do.call(cbind, lapply(1:nfolds_out, function(k) res_E[[17]][[l]][[k]][nIter[[l]][k],])) # dim Nxnfolds
  dev_cov_test[[l]] <- do.call(cbind, lapply(1:nfolds_out, function(k) res_E[[18]][[l]][[k]][nIter[[l]][k],])) # dim Nxnfolds
  dev_geno_cov_test[[l]] <- do.call(cbind, lapply(1:nfolds_out, function(k) res_E[[19]][[l]][[k]][nIter[[l]][k],])) # dim Nxnfolds
  cor_test[[l]] <- do.call(cbind, lapply(1:nfolds_out, function(k) res_E[[20]][[l]][[k]][nIter[[l]][k],])) # dim Nxnfolds
  cor_pval_test[[l]] <- do.call(cbind, lapply(1:nfolds_out, function(k) res_E[[21]][[l]][[k]][nIter[[l]][k],])) # dim Nxnfolds
  
  pred_test <- do.call(cbind, res_E[[22]][[l]])
  adj_exp <- lapply(1:nfolds_out, function(x) t(expDat[Folds[[x]],]) - res_E[[23]][[l]][[x]])
...
write.table(x = cv_train_err, file = sprintf('%s/cv_train_Epar_allchr.txt', outFold), sep = '\t', row.names = F, col.names = T, quote = F)
write.table(x = cv_test_err, file = sprintf('%s/cv_test_Epar_allchr.txt', outFold), sep = '\t', row.names = F, col.names = T, quote = F)
...
save(obj_steps_test, file = sprintf('%s/obj_Epar_cvtest_allchr.RData', outFold))
save(obj_steps_train, file = sprintf('%s/obj_Epar_cvtrain_allchr.RData', outFold))  
save(evalf_steps_test, file = sprintf('%s/evalf_Epar_cvtest_allchr.RData', outFold))
save(evalf_steps_train, file = sprintf('%s/evalf_Epar_cvtrain_allchr.RData', outFold))  
...
train_res_opt <- lapply(1:nfolds_out, function(x) data.frame(dev = dev_train[[id_opt]][,x], dev_geno = dev_geno_train[[id_opt]][,x], 
                                                             ...
                                                             dev_lmgeno = dev_lmgeno_train[[id_opt]][,x], cor = cor_train[[id_opt]][,x], 
                                                             cor_pval = cor_pval_train[[id_opt]][,x]))
...
res_tot_opt <- list(geneAnn = gene_ann, train_opt = train_res_opt, test_opt = test_res_opt, cor_comb_test_opt = cor_test_comb[[id_opt]],
                beta_snps_opt = betaSnpsMatrix[[id_opt]], beta_cov_opt = betaCovMatrix[[id_opt]], weights_opt = weights_opt) 

save(res_tot_opt, file = sprintf('%s/resPrior_EOpt_NestedCV_HeritableGenes_allchr.RData', outFold))
