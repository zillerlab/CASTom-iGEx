...
geneInfo <- read.table(sprintf('%s/resPrior_regEval_allchr.txt', outTrain_fold), header = T, stringsAsFactors = F)
betaCoeff <- get(load(sprintf('%s/resPrior_regCoeffSnps_allchr.RData', outTrain_fold)))
...
for(i in 1:length(all_Chroms)){
  ...
  geneSnpDist <- readMM(sprintf('%s/ENSEMBL_gene_SNP_%s_%s_matrix.mtx', InfoFold, cis_ann, chr))
