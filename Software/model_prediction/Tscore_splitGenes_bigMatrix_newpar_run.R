...
suppressPackageStartupMessages(library(bigmemoryExt))
...
parser$add_argument("--originalRNA", type="logical", default = F, help = "if true, original RNA without filtering used")
...
parser$add_argument("--covDat_file_split", type = "character", nargs = '*',  help = "splitted file annottion sample")
...
covDat_file_split <- args$covDat_file_split
...
# ####################################################################
# input_file <- paste0('/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/OUTPUT_GTEx/predict_CAD/Artery_Coronary/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/split', 1:100, '_predictedExpression_filt.txt')
# # input_file <- '/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/OUTPUT_CMC/predict_UKBB/200kb/devgeno0.01_testdevgeno0/predictedExpression_filt.txt.gz'
# originalRNA <- F
# nFolds <- 10
# perc_comp <- 0.7
# ncores <- 10
# covDat_file <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/covariateMatrix.txt'
# covDat_file_split <- paste0('/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/INPUT_DATA/Covariates/covariates_split', 1:100, '_tmp.txt')
# outFold <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/OUTPUT_GTEx/predict_CAD/Artery_Coronary/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/'
# split_gene_id <- 99
# split_tot <- 100
# ####################################################################
...
colnames_exp <- c()
colnames_exp_list <- vector(mode = 'list', length = length(covDat_file_split))
sampleAnn <- read.table(covDat_file_split[1], header = T, stringsAsFactors = F)
sampleAnn <- sampleAnn[sampleAnn$Individual_ID %in% sampleAnn_tot$Individual_ID,]
eMat <- vector(mode = 'list', length = length(covDat_file_split))
...
for(i in 2:length(input_file)){
  ...
  sampleAnn <- read.table(covDat_file_split[i], header = T, stringsAsFactors = F)
  #print(dim(sampleAnn))
  sampleAnn <- sampleAnn[sampleAnn$Individual_ID %in% sampleAnn_tot$Individual_ID,]
  #print(dim(sampleAnn))
  ...
  if(!identical(geneInfo, expDat[gene_id, colnames(geneInfo)])){print("ERROR: different gene annotation")}
  colnames_exp_list[[i]] <- colnames(expDat[, id_samples])
  #print(length(colnames_exp_list[[i]]))
  colnames_exp <- c(colnames_exp, colnames(expDat[, id_samples]))
...
#registerDoParallel(cores=min(ncores, nFolds))
...
# curValues_ref <- lapply(curReference, function(x) extract_values(inputData, colnames_exp_list, x)[,])
# curValues_comp <- lapply(curCases, function(x) extract_values(inputData, colnames_exp_list, x)[,])
...
for (i in 1:nFolds){
  ...
  comp <- extract_values(inputData, colnames_exp_list, curCases[[i]])
  ...
  # curValues_ref <- extract_values(inputData, colnames_exp_list, curReference)[,]
  # curValues_comp <- extract_values(inputData, colnames_exp_list, curCases)[,]
  print(dim(comp))
  print(dim(ref))
  # res <- matrix(ncol = length(curCases[[i]]), nrow = length(geneNames))
  # res_mat[[i]] <- matrix(ncol = length(curCases[[i]]), nrow = length(geneNames))
  # print(mem_used())
  ...
  #tmp <- parLapply(cl, 1:40, function(j) t_stat(comp[,j] - ref))
  stopCluster(cl)
  
  res_mat[[i]] <- do.call(cbind, tmp)
  
  # for(j in 1:length(curCases[[i]])){
  #   if(j%%1000 == 0){print(paste('sample', j))}
  #   tmp <- curValues_comp[[i]][,j] - curValues_ref[[i]]
  #   res[,j] <- t_stat(tmp)
  # }
  ...
  #   print(j)
  #   # res[j,] <- sapply(curValues_comp[j,], function(x) t_stat_vect(x - curValues_ref[j,]))
  #   for(k in 1:ncol(curValues_comp)){
  #      # print(k)
  #      res[j,k] <- t_stat_vect(curValues_comp[j,k] - curValues_ref[j,])
  #   }
  #   # 
  # }

  # curValues = inputData[,curIds %in% colnames_exp]
...
cl <- makeCluster(ncores)
clusterExport(cl, "ovMat")
clusterExport(cl, "res_mat")
clusterExport(cl, "allSamp")
clusterExport(cl, "geneNames")
clusterExport(cl, "reference")

tmp <- parLapply(cl, 1:length(allSamp), function(s){
  if(allSamp[s] %in% reference){
    if(any(ovMat[rownames(ovMat) %in% allSamp[s],])){
      new <- sapply(res_mat[which(ovMat[rownames(ovMat) %in% allSamp[s],])], function(x) x[,colnames(x) == allSamp[s]])  
      rowMeans(matrix(new, ncol = length(which(ovMat[rownames(ovMat) %in% allSamp[s],])), nrow = length(geneNames)))
  }else{
    rep(NA,length(geneNames))
    }
  }else{
   new <- sapply(res_mat, function(x) x[,colnames(x) == allSamp[s]]) 
...
colnames(tscoreTables) <- allSamp
tscoreTables <- data.frame(geneId = geneNames, tscoreTables)


# tscoreTables <- matrix(nrow = length(geneNames), ncol = length(allSamp), NA)
#  
# for(s in 1:length(allSamp)){
#    
#    #if(s%%1000 == 0){print(paste('sample', s))}
#    print(s)
#   
#    if(allSamp[s] %in% reference){
#     if(any(ovMat[rownames(ovMat) %in% allSamp[s],])){
#       tmp <- sapply(res_mat[which(ovMat[rownames(ovMat) %in% allSamp[s],])], function(x) x[,colnames(x) == allSamp[s]])  
#       # tmp <- do.call(cbind, tmp)
#       tmp <- matrix(tmp, ncol = length(which(ovMat[rownames(ovMat) %in% allSamp[s],])), nrow = length(geneNames))
#       print(dim(tmp))
#       tscoreTables[,s] <- rowMeans(tmp)
#     }
#    }else{
#      tmp <- sapply(res_mat, function(x) x[,colnames(x) == allSamp[s]]) 
#      tscoreTables[,s] <- rowMeans(tmp)
#    }
# }
#  
...
# save results (.RData)
save(tscoreTables, file = sprintf('%s/predictedTscores_splitGenes%i.RData', outFold, split_gene_id))
